...
StatusNet.TabbedMenuBar.prototype.setSelectedTab = function(index) {
    ...
    if (!this.loading) {
        ...
        StatusNet.debug("We're loading a timeline! Can't change tabs!");
    }
};
...
StatusNet.TabbedMenuBar.prototype.highlightTab = function(index) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (i === index) {
            ...
            minitab.backgroundImage = minitab.deselectedImage;
        }
    }
};
...
StatusNet.TabbedMenuBar.prototype.createMiniTab = function(args) {
    ...
    var minitab = Ti.UI.createImageView({
        // Using backgroundImage here since it scales almost correctly
        // loading the high-res versions on 240dpi Android.
        //
        // For some reason in 1.6, with anyDensity off it seems to want
        // to scale them back down to the medium resolution, and scale
        // it back up when drawing when using the actual native image
        // part of the imageview. anyDensity on is too much trouble.
