...
/**
 * Abstract away the difference between size on the various platforms
 */
StatusNet.AvatarCache.getFileSize = function(file) {

    // XXX: size is a function on Desktop and a property on Mobile
    return (typeof file.size == "function") ? file.size() : file.size;
};
...
StatusNet.AvatarCache.lookupAvatar = function(url, onHit, onMiss, relative) {
    ...
    //    var file_age = Date.now() - (this.getAvatarTimestamp(avatarFile)/1000);
    //    StatusNet.debug('file_age is '+(((file_age/1000)/60)/60) + ' hours');
    //}
    
    // TODO don't always skip!
    if (avatarFile.exists() && StatusNet.AvatarCache.getFileSize(avatarFile) > 0) {
        ...
        StatusNet.HttpClient.fetchFile(
            ...
            function(code, e, client) {
                ...
                if (onMiss) {
                    if (StatusNet.Platform.isMobile()) {
                        StatusNet.error("Couldn't get avatar");
                    } else {
                        StatusNet.error("AvatarCache.lookupAvatar I - using "+StatusNet.Theme.getTheme().getDefaultUserImage());
                        onMiss(StatusNet.Theme.getTheme().getDefaultUserImage());
                    }
                }
            }
        );
