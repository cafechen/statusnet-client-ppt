...
StatusNet.HttpClient.fetchFile = function(url, file, onSuccess, onError) {
    ...
    if (url === null) {
        StatusNet.error("HttpClient.fetchFile called with NULL URL!");
    } else {
        StatusNet.error("HttpClient.fetchFile called with URL: " + url);
    }
    try {
        ...
        client.onreadystatechange = function() {
            if (client.readyState == 4) {
                ...
                if (client.timedOut) {
                    ...
                    onError(client.status, "Error: timeout");

                } else if (client.responseData.length <= 1) {

                    StatusNet.debug("fetchFile: failure!");
                    onError(client.status, "Error: response less than 1");

                } else if (client.status != 200) {

                    StatusNet.debug("fetchFile: failure!");
                    onError(client.status, "Error: "+client.statusText);
                    ...
                    if (file.exists()) {
                        ...
                        onSuccess(client.status, client);
                    } else {
        ...
        client.onerror = function(e) {
            StatusNet.debug("fetchFile: failure!");
            onError(client.status, "Error: " + e.error, client);
        };

        client.open("GET", url, true); // make sure we're async
        client.send();
